...
export default function LoginPageClient({ locale, redirectTo, error, registered }: LoginPageClientProps) {
  ...
  const t = (key: string) => {
    const translations: Record<string, Record<string, string>> = {
      ro: {
        ...
        registeredSuccess: "Cont creat! Verifica email-ul pentru confirmare, apoi te poti autentifica.",
        googleError: "Eroare la autentificarea cu Google",
      },
      en: {
        title: "Login",
        email: "Email",
        ...
        registeredSuccess: "Account created! Check your email for confirmation, then sign in.",
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    await signInWithEmail({ email, password, locale, redirectTo })
  }
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        {error && (
          <div className="rounded-md bg-destructive/10 p-3 text-sm text-destructive text-center">
            {error === "invalid_credentials" ? t("loginError") : t("googleError")}
          </div>
        )}
